...
def evaluate(args, model, test_loader, datareader, device):
    ...
    for idx in range(len(action_clips)):
        ...
        # 保存每帧误差
        all_frame_errors.extend(err1)

    # 统计误差分布
    bins = [-np.inf, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, np.inf]
    bin_labels = ['<25mm', '25-30mm', '30-35mm', '35-40mm', '40-45mm',
                  '45-50mm', '50-55mm', '55-60mm', '60-65mm', '65-70mm', '>70mm']
    all_frame_errors = np.array(all_frame_errors)
    sample_counts = np.histogram(all_frame_errors, bins=bins)[0]
    total_samples = len(all_frame_errors)
    sample_ratios = sample_counts / total_samples

    # 打印误差分布
    print("\n误差分布统计结果：")
    for i in range(len(bin_labels)):
        print(f"{bin_labels[i]}: {sample_counts[i]} 样本 ({sample_ratios[i]:.2%})")
    ...
    for action in action_names:
        mean_mpjpe = np.mean(results[action])
        mean_p_mpjpe = np.mean(results_procrustes[action])
        mean_acceleration = np.mean(results_acceleration[action])

        final_result.append(mean_mpjpe)
        final_result_procrustes.append(mean_p_mpjpe)
        final_result_acceleration.append(mean_acceleration)

        action_level_results['mpjpe'][action] = mean_mpjpe
        action_level_results['p_mpjpe'][action] = mean_p_mpjpe
        action_level_results['acceleration_error'][action] = mean_acceleration
    ...
    for action in action_names:
        mpjpe = action_level_results['mpjpe'][action]
        p_mpjpe = action_level_results['p_mpjpe'][action]
        acceleration_error = action_level_results['acceleration_error'][action]
        print(
            f"Action: {action: <10} | MPJPE: {mpjpe:.2f} mm | P-MPJPE: {p_mpjpe:.2f} mm | Acceleration Error: {acceleration_error:.2f} mm/s^2")

    return e1, e2, joint_errors, acceleration_error, sample_counts, sample_ratios
...
def train(args, opts):
    ...
    # model.to(device)
    model = load_model(args)
    model.to(device)  # 将模型放到cuda:1
    if torch.cuda.is_available():
        model = torch.nn.DataParallel(model, device_ids=[1])  # 指定使用 cuda:1
    ...
    checkpoint_path_best = os.path.join(opts.new_checkpoint, 'base-243-10-delete', 'best_epoch.pth.tr')
    checkpoint_path_latest = os.path.join(opts.new_checkpoint, 'base-243-10-delete', 'latest_epoch.pth.tr')
